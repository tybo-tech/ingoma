...
export class AddProductComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.existingProduct.ProductId) {
      ...
      this.product.CreateUserId = ZOWEH;
      this.product.ModifyUserId = ZOWEH;
  ...
  saveProduct() {
    this.product.ProductSlug = this.productService.generateSlug(ZOWEH, this.product.Name, '');
  ...
  public uploadMusic = (files: FileList) => {
    debugger
    if (files.length === 0) {
      return;
    }
    ...
    Array.from(files).forEach(file => {
      ...
      formData.append('name', `tybo.${file.name.split('.')[file.name.split('.').length - 1]}`); // file extention
      this.uploadService.uploadFile(formData).subscribe(url => {
        this.product.Code = `${environment.API_URL}/api/upload/${url}`;
      });
